...
export const formValidationScheme = Yup.object().shape({
    movieTitle: Yup.string().trim()
            .required('Edite o título do filme')
            .max(50, 'O título do filme não pode ter mais de 50 caracteres'),
    movieGenre: Yup.string().trim()
            .required('Edite o gênero do filme!'),
    duration: Yup.string().trim()
            .required('Edite a duração do filme'),
    movieLink: Yup.string().trim()
        .required('Edite o link do filme'),
    country: Yup.string().trim()
        .required('Edite o país de origem do filme'),
    file: Yup.mixed<Blob>()
            .required('Edite o poster do filme!')
